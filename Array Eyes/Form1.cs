        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (checkBox5.Checked == true)
                {
                    ...
                    Sql = "select SingleName,count(SingleName) as Count from Map_SingleID  where 1=1 ";
                    ...
                    Sql = Sql + " GROUP BY SingleName order by SingleName";
                    ReadFromSQL(Sql, "Map_SingleID", 5);

                    Sql = "select Model,batch,LEFT(Q_ID,4)+SUBSTRING(Q_ID,13,1) AS Q_Name,sum(cast(SingleCount as bigint)) as Count from "
                          + "(select distinct  LotID,Model,batch,Q_ID,SingleCount from Map_SingleID where 1=1 ";
                    if (checkBox1.Checked == true)
                        Sql = Sql + " and Date BETWEEN " + StartTime + " and " + EndTime;
                    if (checkBox2.Checked == true)
                    {
                    ...
                    Sql = Sql + ") as m1 group by Model,batch,LEFT(Q_ID,4)+SUBSTRING(Q_ID,13,1) order by Model";
                   // MessageBox.Show("3");
